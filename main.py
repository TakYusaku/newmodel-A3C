...
from optim import AsyncRMSprop, SharedRMSprop, SharedAdam
...
if __name__ == '__main__':
    ...
    parser.add_argument('--log_dir', type=str, default='./log_dir',
                        help='save dir')
    parser.add_argument('--epoch', type=int, default=4500, metavar='N',
                        help='training epoch number') #default=10000000
    parser.add_argument('--local_t_max', type=int, default=20, metavar='N',
                        help='bias variance control parameter')
    parser.add_argument('--entropy_beta', type=float, default=0.01, metavar='E',
    ...
    optimizer = SharedAdam(global_brain.parameters(), lr=args.lr, betas=(0.92, 0.999))
    #optimizer.share_memory()
    global_ep, global_ep_r, res_queue = mp.Value('i', 0), mp.Value('d', 0.), mp.Queue()
    
    wt = Environment(args, global_ep, global_ep_r, res_queue, global_brain, optimizer)
    ...
    for rank in range(args.num_process):
        get_rev,send_rev  = mp.Pipe(False)
        p = mp.Process(target=wt.cartpole_train_3_5, args=(rank, args))
        processes.append(p)
        pipe_reward.append(get_rev)
        p.start()
